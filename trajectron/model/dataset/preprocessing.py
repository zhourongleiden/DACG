...
def get_node_timestep_data(env, scene, t, node, state, pred_state,
    ...
    # mean_position of this scene
    mp = scene.mean_position()
    mp_t = torch.tensor(mp, dtype=torch.float)
    
    # Node
    timestep_range_x = np.array([t - max_ht, t])
    timestep_range_y = np.array([t + 1, t + max_ft])

    x = node.get(timestep_range_x, state[node.type], padding=np.nan)
    ...
    neighbors_data_y = None
    neighbors_data_y_st = None  
    neighbors_all = None      
    ...
    if hyperparams['edge_encoding']:
        ...
        neighbors_data_y = dict()
        neighbors_data_y_st = dict()   
        neighbors_all = dict()
        neighbors_edge_value = dict()
        first_history_index_neighbors = dict()
        first_future_index_neighbors = dict()
        
        for edge_type in edge_types:
            neighbors_data[edge_type] = list()
            neighbors_data_st[edge_type] = list()  
            neighbors_data_y[edge_type] = list()
            neighbors_data_y_st[edge_type] = list()
            neighbors_all[edge_type] = list()
            first_history_index_neighbors[edge_type] = list()
            first_future_index_neighbors[edge_type] = list()
            ...
            first_history_index_connected_nodes = []
            first_future_index_connected_nodes = []
            for connected_node in connected_nodes:
                neighbor_state_np = connected_node.get(np.array([t - max_ht, t]), state[connected_node.type], padding=np.nan)
                neighbor_state_np_y = connected_node.get(np.array([t + 1, t + max_ft]), pred_state[connected_node.type], padding=0.0)
                neighbor_state_all = connected_node.get(np.array([t - max_ht, t + max_ft]), pred_state[connected_node.type], padding=np.nan)                
                first_history_index_connected_nodes.append((max_ht - connected_node.history_points_at(t)).clip(0))
                first_future_index_connected_nodes.append((min(connected_node.future_points_at(t), max_ft-1)))
                ...
                neighbor_state_np_st = env.standardize(neighbor_state_np,
                                                       ...
                                                       mean=rel_state,
                                                       std=std)
                neighbor_state_np_y_st = env.standardize(neighbor_state_np_y,
                                                       pred_state[connected_node.type],
                                                       node_type=connected_node.type,
                                                       mean=rel_state[0,0:2])               
                ...
                neighbor_state_y_st = torch.tensor(neighbor_state_np_y_st, dtype=torch.float)
                neighbors_data_y_st[edge_type].append(neighbor_state_y_st)
                
                ###############################################################
                neighbor_state = torch.tensor(neighbor_state_np, dtype=torch.float)
                neighbors_data[edge_type].append(neighbor_state)
                neighbor_state_y = torch.tensor(neighbor_state_np_y, dtype=torch.float)
                neighbors_data_y[edge_type].append(neighbor_state_y)      
                neighbor_state_all = torch.tensor(neighbor_state_all, dtype=torch.float)
                neighbors_all[edge_type].append(neighbor_state_all)  
                ###############################################################
                
            first_history_index_connected_nodes = torch.tensor(first_history_index_connected_nodes, dtype=torch.float)
            first_history_index_neighbors[edge_type].append(first_history_index_connected_nodes)
            first_future_index_connected_nodes = torch.tensor(first_future_index_connected_nodes, dtype=torch.float)
            first_future_index_neighbors[edge_type].append(first_future_index_connected_nodes)            
    ...
    return (first_history_index, x_t, y_t, x_st_t, y_st_t, first_history_index_neighbors, first_future_index_neighbors,
            neighbors_data_st, neighbors_data_y_st, neighbors_data, neighbors_data_y, neighbors_all, mp_t)
...
def get_timesteps_data(env, scene, t, node_type, state, pred_state,
    ...
    :return:
    """
    #寻找有效node,testing时保证长度大于等于7+1+12=20
